...
- **Mechanics:** `COMPONENT-MIGRATION-PLAYBOOK.md`. The rail is now **manifest-driven** — the ordered IA lives in `manifest.json` → `nav` (sections → collapsible category groups → leaves, each `{label, ref, status}`; `status: "planned"` marks a stub). You no longer hand-edit `partials/nav.html` or any `targetList`/`CURRENT` table. Per-cycle wiring is just: point the component's `ref` at `components/<slug>/` (and drop its `status: "planned"`) in `manifest.json`, build the `wire-doc-pager` chain — then `node scripts/build-ia.mjs` regenerates `partials/nav.html`, stubs any still-missing pages, derives the target list + current-page map, and fills nav/footer between the `NAV/FOOTER` markers (stamping the current section **and** category group open). `node scripts/sync-nav.mjs` is a back-compat shim that just calls `build-ia.mjs`. Clone the page scaffold from a recent component (`components/search/index.html`).
- **Migrated groups (31 pages):** Actions, Forms & inputs, Selection & status, Containment & overlays, Navigation, Content & display, Messaging & feedback (Toast, Banner, Empty state, Stepper, Tooltip, Skeleton — `docs/feedback.html` retired from the IA, kept on disk).
- **Remaining:** Layout (`docs/content-blocks.html` / `heroes.html` / `footers.html` — overlaps the Foundations `docs/layout.html`), Patient affordances (`docs/research.html`).
